                        ...
                        while endpoint < len(code_line):
                            if code_line[cursor] != ' ':
                                ...
                                if chunk[0] == 'G':
                                    ...
                                    drillmacro['Z'].append(chunk[1:])
                                elif chunk[0] == 'R':
                                    drillmacro['R'].append(chunk[1:])
                                elif chunk[0] == 'J':
                                    drillmacro['J'].append(chunk[1:])
                                elif chunk[0] == 'F':
                                    pass
                            cursor = endpoint
                            endpoint = cursor + 1
        ...
        def adjust_tapping_code(event):
            for tool, values in rigid_tappingdict.items():
                ...
                if multiplier == '.33':
                    ...
                    pass
                final_depth = int(final_tap_elements[tool][1]['Z'][0])
                for element in linesToAdjust:
                    current_zdepth = int(f'{final_depth / passes: .4f}')
                    if 'G84' in element:
                        if 'X' not in element and 'Y' not in element:
                            initial_line = f'G84 Z{str(current_zdepth)} R{str(final_tap_elements[tool][1]['R'][0])} J{str(final_tap_elements[tool][1]['J'][0])} F{str(final_tap_elements[tool][1]['F'][0])}'
                        elif 'X' in element and 'Y' not in element:
                            cursor = element.index('X')
                            while element[cursor] != ' ':
                                cursor += 1
                            first_half = element[:cursor]
                            second_half = f'Z{str(current_zdepth)} R{str(final_tap_elements[tool][1]["R"][0])} J{str(final_tap_elements[tool][1]["J"][0])} F{str(final_tap_elements[tool][1]["F"][0])}'
                            initial_line = first_half + second_half
                        elif 'X' not in element and 'Y' in element:
                            cursor = element.index('Y')
                            while element[cursor] != ' ':
                                cursor += 1
                            first_half = element[:cursor]
                            second_half = f'Z{str(current_zdepth)} R{str(final_tap_elements[tool][1]["R"][0])} J{str(final_tap_elements[tool][1]["J"][0])} F{str(final_tap_elements[tool][1]["F"][0])}'
                            ...
                            if element.index('Y') > element.index('X'):
                                cursor = element.index('Y')
                            else:
                                cursor = element.index('X')
                            while element[cursor] != ' ':
                        ...
                        while current_zdepth <= final_depth:
                            tapping_adjustments += f''
                    elif 'G80' not in element:
